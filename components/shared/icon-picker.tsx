...
const CDN = 'https://cdn.jsdelivr.net/gh/selfhst/icons@main'
...
export function iconUrl(reference: string) {
  return `${CDN}/webp/${reference}.webp`
}
...
export function IconPicker({ value, onChange }: IconPickerProps) {
  ...
  const filtered = query.trim()
    ? icons.filter(i => i.Name.toLowerCase().includes(query.toLowerCase()) || i.Reference.toLowerCase().includes(query.toLowerCase())).slice(0, 40)
    : icons.slice(0, 40)
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-50 w-80 rounded-xl border border-border bg-popover shadow-xl">
              ...
              <div className="grid grid-cols-5 gap-1">
                {filtered.map(icon => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 p-1.5 rounded-lg hover:bg-muted transition-colors group ${value === icon.Reference ? 'bg-muted ring-1 ring-primary' : ''}`}
                  >
                    <img
                    ...
                    />
                    <span className="text-[9px] text-muted-foreground truncate w-full text-center leading-tight">{icon.Name}</span>
                  </button>
                ))}
              </div>
            )}
