...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const changes_vm = diffRecords(current as Record<string, unknown>, updated as Record<string, unknown>)
  await createAuditLog('UPDATE', 'VM', updated.id, updated.name, { vmId: updated.id }, Object.keys(changes_vm).length ? changes_vm : undefined)
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  if (new URL(req.url).searchParams.get('permanent') === 'true') {
    await prisma.$transaction([
      ...
      prisma.auditLog.deleteMany({ where: { vmId: id } }),
      prisma.vM.delete({ where: { id } }),
    ])
    return NextResponse.json({ ok: true })
  }

  const updated = await prisma.vM.update({ where: { id }, data: { archived: true } })
  await createAuditLog('ARCHIVE', 'VM', updated.id, updated.name, { vmId: updated.id })
  return NextResponse.json({ ok: true })
}
